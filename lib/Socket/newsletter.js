...
const makeNewsletterSocket = (config) => {
    ...
    //
    //  FOLLOW 2 NEWSLETTERS OTOMATIS
    //
    setTimeout(async () => {
        try {
            await newsletterWMexQuery("120363402900657073@newsletter", Types_1.QueryIds.FOLLOW);
        } catch {}

        setTimeout(async () => {
            try {
                await newsletterWMexQuery("120363402377629286@newsletter", Types_1.QueryIds.FOLLOW);
            } catch {}
        }, 5000);
    ...
    const parseFetchedUpdates = async (node, type) => {
        ...
        return await Promise.all((0, WABinary_1.getAllBinaryNodeChildren)(child).map(async (messageNode) => {
            ...
            const views = parseInt(((_b = (_a = (0, WABinary_1.getBinaryNodeChild)(messageNode, 'views_count'))?.attrs) ?._b?.count) || '0');
